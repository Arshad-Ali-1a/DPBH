...
@app.get("/")
def read_root():
    return {"Hello": "World", "endpoints":["/extract_text","/detect-darkness"]}
...
async def json_input(data: dict,response:Response):
    ...
    highlighted_img = Image.fromarray(highlighted_img_data)
    ...
    print("img_str: ",len(img_str) if img_str else "EMPTYYY",flush=True)
    return {"highlighted_img_data":img_str, "dark_patterns":res_dark}
